...
const debridKey = process.env.DEBRID_API_KEY;
const debridProvider = process.env.DEBRID_PROVIDER;

// ============================================================
// MANIFEST DO ADDON (SIMPLIFICADO - stream DIRETO)
// ============================================================
...
const manifest = {
  ...
  version: '1.0.0',
  name: 'BluDV (stream)',
  description: 'Filmes e Séries do BluDV. Conteúdo em Português com Dual Áudio. (Apenas stream)',
...
// Stream: Links stream diretos
builder.defineStreamHandler(async ({ type, id }) => {
  console.log(`[Stream] type=${type} id=${id}`);
  if (!id.startsWith('bludv:')) return { streams: [] };

  // Parsear episódio do ID (ex: bludv:12345:ep5 ou bludv:12345:1:5)
  ...
  if (parts[1]) {
    if (parts[1].startsWith('ep')) {
      episode = parseInt(parts[1].replace('ep', ''));
    } else {
